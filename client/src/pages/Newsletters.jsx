  ...
  const handleSendNewsletter = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/otherroutes/admin/send-mail",
        {
          method: "POST",
          headers: {
    ...
    <div className="flex flex-col items-center p-6 bg-gray-100 min-h-screen mt-10">
      <div className="w-full max-w-2xl bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-bold text-center mb-6">
          SehatStation Newsletter Dashboard
        </h2>
        ...
        {status && (
          <div
            className={`mt-4 p-3 rounded ${status.type === "success" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
          >
            {status.message}
          </div>
        )}
